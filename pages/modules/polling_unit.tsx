...
const PollingUnit: NextPage<{
    data: {
        payload: IPollingUnit[]; page: number;
        total: number;
        size: number;
        rows: number;
    ...
    const [pollingUnits, setPollingUnits] = React.useState<IPollingUnit[]>([])
    ...
    const tableData = React.useMemo(() => pollingUnits, [pollingUnits]);

    const columns: Column<IPollingUnit>[] = React.useMemo(
        () => [
            {
                Header: 'Polling Unit Number',
                accessor: 'polling_unit_number', // accessor is the "key" in the data
            },
            {
                Header: 'Polling Unit Name',
                accessor: 'polling_unit_name',
            },
            {
                Header: 'Latitude',
                accessor: 'lat'
    ...
    const memo = React.useCallback(
        async () => {
            ...
            let ans = await Api.getModelList<IPollingUnit>({
    ...
    const { data: ret, isFetching, refetch, isError } = useQuery('load-polls', memo, {
        enabled,
                                                ...
                                                return (
                                                    <td {...cell.getCellProps()} className="py-2 pr-2 font-medium text-sm leading-6 whitespace-nowrap border-t border-secondary dark:border-otherColor">
                                                        ...
                                                        <td  className="py-2 text-cyan-500 pr-2 font-medium text-sm leading-6 whitespace-nowrap border-t border-secondary dark:border-otherColor">
                                                          <Link href={'/modules/polling_unit_result/' + pollingUnits[i].uniqueid} target="_blank">  
                                                          View Results
                                                                </Link>
                                                        </td>
                                    </tr>
...
export async function getServerSideProps() {
    ...
    const res = await fetch(`${SERVER_URL}/get-items/polling_unit`)
...
export default WithSideBar(PollingUnit, 'polling_unit');
